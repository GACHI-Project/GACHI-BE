 ...
 *
 * 흐름 요약:
 * 1. 가정통신문 AI 분석 완료 → AI 파이프라인에서 Redis에 preview 데이터 저장 (추후 연결)
 * 2. GET /calendar/preview → Redis에서 읽어 팝업에 표시
 * 3. PATCH /calendar/dates → Redis에서 날짜 수정 후 다시 저장
 * 4. POST /calendar → Redis 데이터 기반으로 calendar_events insert → Redis 키 삭제
 *
...
public class CalendarPreviewRedisService {
    ...
    public List<CalendarPreviewEvent> getPreview(Long newsletterId) {
        ...
        try {
            ...
            log.debug("[CalendarPreview] preview 조회 성공. newsletterId={}, count={}", newsletterId, events.size());
            return events;
        } catch (Exception e) {
            log.error("[CalendarPreview] Redis 데이터 파싱 실패. newsletterId={}, error={}", newsletterId, e.getMessage());
            return null;
        }
    }
    ...
    public void savePreview(Long newsletterId, List<CalendarPreviewEvent> events) {
        ...
        try {
            ...
            log.debug("[CalendarPreview] preview 저장 완료. newsletterId={}, count={}", newsletterId, events.size());
        } catch (Exception e) {
            log.error("[CalendarPreview] Redis 저장 실패. newsletterId={}, error={}", newsletterId, e.getMessage());
