...
public class CalendarPreviewMockService {
    ...
    /**
     * Redis에 preview 더미 데이터 주입.
     * tempEventId는 자동 생성
     */
    public void injectMockPreview(Long userId, Long newsletterId,
                                  CalendarPreviewMockRequest request) {
        ...
        boolean needsAutoAssign = mockEvents.stream()
            .anyMatch(e -> e.checklistIds() == null || e.checklistIds().isEmpty());

        // 자동 배분 필요 시 DB에서 CHECKLIST 타입 항목 ID 조회
        List<Long> allChecklistIds = List.of();
        if (needsAutoAssign) {
            allChecklistIds = checklistRepository
        ...
        for (int i = 0; i < eventCount; i++) {
            ...
            previewEvents.add(new CalendarPreviewEvent(
                tempEventId,
                mockEvent.title(),
            ...
            log.debug("[MockPreview] 이벤트 생성. tempEventId={}, title={}, checklistIds={}",
                tempEventId, mockEvent.title(), checklistIds);
        }

        // Redis에 저장
        previewRedisService.savePreview(newsletterId, previewEvents);

        log.info("[MockPreview] Redis 주입 완료. userId={}, newsletterId={}, eventCount={}",
            userId, newsletterId, previewEvents.size());
    }
}
