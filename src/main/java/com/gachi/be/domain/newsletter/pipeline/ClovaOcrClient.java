 ...
 *
 * <p>호출 방식: Presigned URL 방식
 * 파일을 Base64로 인코딩하는 대신 S3 Presigned URL을 클로바에 전달하고
 * 클로바가 직접 S3에서 파일을 가져가는 방식.
 *
 * <p>PDF 지원:
 * 클로바 OCR은 PDF를 직접 지원하므로 여러 페이지 가정통신문도 1회 호출로 처리 가능.
 * format 필드를 "jpeg"/"png"/"pdf" 중에서 파일 형식에 맞게 지정하면 된다.
 *
 * <p>ObjectMapper:
 * Spring Boot가 자동으로 빈으로 등록하므로 @RequiredArgsConstructor로 주입받아 사용.
 * IDE에서 빨간 줄이 생길 수 있으나 실제 실행 시에는 정상 주입된다.
 */
@Slf4j
@Component
...
public class ClovaOcrClient {
    ...
    /**
     * S3에 저장된 가정통신문 파일에 대해 클로바 OCR을 호출.
     * 파일 형식에 따라 클로바 format 값을 자동으로 결정.
     * - PDF: "pdf" → 클로바가 내부적으로 전 페이지 처리
     * - jpg/jpeg: "jpeg"
     * - png: "png"
    ...
    /**
     * file_key의 확장자로 클로바 OCR format 값을 결정
     * 클로바 OCR 허용 format 값: "jpeg", "png", "pdf", "tiff"
     * PDF는 클로바가 전 페이지를 처리하므로 별도 변환 불필요.
     */
    ...
    private String buildRequestBody(String presignedUrl, String format) {
        try {
            ...
            throw new ExternalApiException(ErrorCode.EXTERNAL_API_ERROR,
                "클로바 OCR 요청 본문 구성 실패: " + e.getMessage());
        }
    }
    ...
    private String executeHttpRequest(String requestBody) {
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                log.error("[ClovaOcr] API 호출 실패. status={}, body={}",
                    response.statusCode(), response.body());
                throw new ExternalApiException(ErrorCode.EXTERNAL_API_ERROR,
                    "클로바 OCR API 오류. status=" + response.statusCode());
            }
            ...
            throw new ExternalApiException(ErrorCode.EXTERNAL_API_ERROR,
                "클로바 OCR API 통신 오류: " + e.getMessage());
        }
    }

    /**
     * 클로바 OCR 응답에서 fields 리스트를 추출.
     * PDF의 경우 여러 페이지가 images 배열에 각각 들어오므로 모든 images의 fields를 합쳐서 반환.
     */
    private List<OcrField> parseFields(String responseBody, String fileKey) {
        try {
            ...
            if (response.images() == null || response.images().isEmpty()) {
                throw new ExternalApiException(ErrorCode.EXTERNAL_API_ERROR,
                    "클로바 OCR 응답에 images 배열이 없습니다.");
            }

            // 모든 페이지(images)의 fields를 하나의 리스트로 합침
            // PDF 여러 페이지인 경우 각 페이지 fields가 순서대로 합쳐짐
            List<OcrField> allFields = response.images().stream()
                .peek(image -> {
                    if (!"SUCCESS".equals(image.inferResult())) {
                        log.warn("[ClovaOcr] 페이지 인식 실패. inferResult={}, fileKey={}",
                            image.inferResult(), fileKey);
                    }
                })
            ...
            if (allFields.isEmpty()) {
                throw new ExternalApiException(ErrorCode.EXTERNAL_API_ERROR,
                    "클로바 OCR 인식 결과가 없습니다. 모든 페이지가 실패했습니다.");
            }
     ...
     * OCR 인식 결과 단위 (텍스트 블록).
     * inferText: 인식된 텍스트
     * boundingPoly: 텍스트 박스의 꼭짓점 좌표 (Y좌표 정렬에 사용)
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
    @Getter
    @NoArgsConstructor
