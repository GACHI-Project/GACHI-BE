...
public class Newsletter {
  ...
  /** AWS S3에 저장된 파일의 오브젝트 키(경로). Presigned URL 생성 시 + 클로바 OCR URL 전달에 사용 예정 */
  @Column(name = "file_key", nullable = false, length = 500)
  private String fileKey;

  /** 파일 내용의 SHA-256 해시값. 동일한 파일의 중복 업로드를 방지하기 위해 사용. Partial Unique Index 사용 */
  @Column(name = "file_hash", nullable = false, length = 64)
  private String fileHash;

  /** AI 분석 파이프라인의 현재 진행 상태. PENDING → PROCESSING → COMPLETED / FAILED 순 */
  ...
  /** 전처리·정제된 최종 원문. ocr_text에서 노이즈 제거·공백 정규화 후처리를 거친 결과. 번역·요약의 실제 입력값으로 사용 예정. */
  @Column(name = "original_text", columnDefinition = "TEXT")
  private String originalText;

  /** 번역된 텍스트. language=KO인 경우 NULL (번역 스킵). 그 외 파파고 API 번역 결과. */
  @Column(name = "translated_text", columnDefinition = "TEXT")
  private String translatedText;

  /** AI가 추출한 가정통신문 제목. KO: 원문 제목 / 그 외: 번역된 제목. 문서 목록 화면에 표시. */
  @Column(name = "title", length = 255)
  private String title;

  /** AI 생성 요약문. OpenAI API로 생성 예정. */
  ...
  /**
   * 파일 업로드 시점에 newsletter 레코드를 생성하는 빌더. AI 결과 컬럼들(ocrText 등)은 업로드 시점에 NULL이며 비동기 AI 파이프라인에서 채울 예정.
   */
  @Builder
  public Newsletter(
      Long userId,
