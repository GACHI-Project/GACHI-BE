...
/**
 * 클로바 OCR 결과를 파싱하고 텍스트를 정제하는 컴포넌트.
 * 1. 파싱
 * 2. 정제
 */
@Slf4j
...
public class OcrTextRefiner {
    ...
    public String parseFields(List<OcrField> fields) {
        ...
        for (TextBlock block : blocks) {
            if (prev == null) {
                ...
                double threshold = prev.height() > 0
                    ? prev.height() * 0.7
                    : 15.0;
        ...
        log.debug("[OcrTextRefiner] 파싱 완료. blocksCount={}, textLength={}",
            blocks.size(), result.length());
    ...
    public String refineText(String rawText) {
        ...
        log.debug("[OcrTextRefiner] 정제 완료. before={}chars, after={}chars",
            rawText.length(), refined.length());
        return refined;
    }

    /**
     * OcrField의 boundingPoly에서 Y좌표 중앙값을 계산
     * 4개 꼭짓점의 Y좌표 평균을 내어 텍스트 블록의 중심 Y값
     * vertices가 없으면 0.0 반환 → 맨 위에 배치.
     */
