...
public class ImagePreprocessor {
    ...
    /**
     * EXIF 메타데이터의 회전 정보를 읽어 이미지를 실제로 회전시킴
     * EXIF Orientation 값:
     * - 1: 정상 (회전 없음)
     * - 3: 180도 회전
     * - 6: 시계 방향 90도 회전 (핸드폰 세로 촬영 후 가로로 찍힌 경우)
     * - 8: 반시계 방향 90도 회전
     * EXIF 읽기에 실패하더라도 원본 이미지를 그대로 반환하여 OCR 파이프라인 자체가 중단되지 않도록 처리
     */
    ...
    /**
     * EXIF 메타데이터에서 Orientation 값을 읽음
     * 실패 시 기본값 1(정상)을 반환하여 파이프라인이 계속 진행되도록 함
     * PNG 파일은 대부분 EXIF 정보가 없으므로 예외가 발생하는 것이 정상.
     */
    ...
    /**
     * BufferedImage를 지정한 각도(degrees)만큼 회전시킴
     * 90도/270도 회전 시 가로/세로가 바뀌므로 새 이미지의 width/height를 반전시켜야 함.
     */
    private BufferedImage rotate(BufferedImage original, int degrees) {
        ...
        g2d.translate((newWidth - original.getWidth()) / 2.0,
            (newHeight - original.getHeight()) / 2.0);
    ...
    /**
     * BufferedImage를 PNG 바이트 배열로 변환함.
     * 클로바 OCR API로 전달할 때 PNG를 사용함.
     * JPEG는 손실 압축이 발생할 수 있어 텍스트 인식률에 영향을 줄 수 있으므로 PNG 선택.
     */
