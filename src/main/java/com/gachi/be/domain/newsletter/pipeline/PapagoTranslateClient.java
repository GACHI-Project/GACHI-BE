...
public class PapagoTranslateClient {
     ...
     * 한국어 텍스트를 대상 언어로 번역.
     * KO이면 번역 스킵 → null 반환.
     * DB에 translated_text = NULL로 저장되어 프론트는 originalText를 그대로 표시.
     */
    public String translate(String originalText, String targetLanguage) {
        ...
        log.debug("[Papago] 번역 시작. targetLanguage={}, papagoCode={}",
            targetLanguage, papagoTargetCode);
    ...
    private String executeTranslation(String text, String papagoTarget) {
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                log.error("[Papago] API 호출 실패. status={}, body={}",
                    response.statusCode(), response.body());
                throw new ExternalApiException(ErrorCode.EXTERNAL_API_ERROR,
                    "파파고 번역 API 오류. status=" + response.statusCode());
            }
