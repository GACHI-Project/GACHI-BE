...
public class ChecklistServiceImpl implements ChecklistService {
    ...
    public ChecklistTodayResponse getTodayChecklists(Long userId) {
        ...
        List<CalendarEvent> todayEvents = calendarEventRepository.findEventsInRange(
        ...
        // 오늘 일정 ID 목록 추출
        List<Long> todayEventIds = todayEvents.stream()
            .map(CalendarEvent::getId)
            .toList();

        //해당 일정들에 연결된 미완료 CHECKLIST 조회
        List<Checklist> checklists = checklistRepository
            .findIncompleteChecklistsByCalendarEventIds(userId, todayEventIds);
        ...
        Map<Long, CalendarEvent> eventMap = todayEvents.stream()
            .collect(Collectors.toMap(CalendarEvent::getId, e -> e));

        // 체크리스트 → DTO 변환 (newsletter 제목 + 자녀 이름 조회)
        List<ChecklistTodayItem> items = checklists.stream()
            .map(c -> {
                // 가정통신문 제목 조회
                String newsletterTitle = newsletterRepository.findById(c.getNewsletterId())
    ...
    public void updateComplete(Long userId, Long checklistId, ChecklistCompleteRequest request) {
        ...
        log.debug("[ChecklistService] 완료 처리. userId={}, checklistId={}, isCompleted={}",
            userId, checklistId, request.isCompleted());
    }
    ...
    public void deleteChecklist(Long userId, Long checklistId) {
        ...
        log.info("[ChecklistService] 체크리스트 삭제. userId={}, checklistId={}, type={}",
            userId, checklistId, checklist.getType());
    }
}
